...
function CartDAO(database) {
                ...
                { item: 
                    { $filter: 
                        { 
                            input: "$items", 
                            as: "item", 
                            cond: { $eq: [ "$$item._id", itemId ] } 
                        } 
                    } 
                } 
            }]).toArray(function(err, result){
                if(result[0].item.length > 0){
                    console.log("result ", result[0].item[0]);
                    callback(result[0].item[0]);
                } else
                    callback(null);
            });
    ...
    this.addItem = function(userId, item, callback) {
        ...
        this.db.collection("cart").findOneAndUpdate(
            ...
            // if one doesn't alrVBeady exist (i.e. "upsert: true") and that
